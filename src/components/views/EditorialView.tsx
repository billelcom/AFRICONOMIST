...
export const EditorialView: React.FC<EditorialViewProps> = ({
  ...
  // Automated 30-minute ingestion cycle state (مربوط بتوقيت الساعة العالمي الحقيقي)
  ...
  // مزامنة العداد التنازلي للعرض (تم تجميد التشغيل التلقائي مؤقتاً بناءً على الطلب)
  useEffect(() => {
    const syncTime = () => {
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 flex flex-col justify-between gap-3 shadow-sm">
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-amber-400">
                      {isAr ? 'النوع الأول: تقارير الرصد الآلي (مجمّد مؤقتاً)' : 'Type 1: Autonomous Pulse (Paused)'}
                    </span>
                  </div>
                ...
                <h3 className="text-sm font-black text-white">
                  {isAr ? 'توليد فوري للمسودات الآلية (عشوائي شامل)' : 'Instant Autonomous Draft Generation'}
                </h3>
                <p className="text-xs text-slate-400 leading-relaxed">
                  {isAr 
                    ? 'تم تجميد الدورة التلقائية مؤقتاً. اضغط على زر "توليد فوري الآن" لإنشاء مسودة عشوائية فورية تشمل الـ 54 دولة و 28 قطاعاً و 18 قالباً صحفياً.' 
                    : 'Periodic auto-pulse is paused. Use Instant Generate Now to commission a random draft across 54 countries and 28 sectors.'}
                </p>
              </div>
              ...
              <div className="pt-2 flex items-center justify-between gap-2 border-t border-slate-800/80">
                <span className="text-[11px] text-slate-400 flex items-center gap-1.5">
                  ...
                  {isAr ? 'جاهز للتوليد الفوري بنقرة واحدة' : 'Ready for 1-click instant generation'}
                </span>
                <button
                  type="button"
