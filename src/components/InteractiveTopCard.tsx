...
interface InteractiveTopCardProps {
  lang: 'ar' | 'en';
  className?: string;
}

export const InteractiveTopCard: React.FC<InteractiveTopCardProps> = ({
  lang,
  className = ''
}) => {
  const isAr = lang === 'ar';
  ...
  const [returnCountdown, setReturnCountdown] = useState<number>(120);
  ...
  // العودة التلقائية للساعة بعد دقيقتين (120 ثانية)
  useEffect(() => {
  ...
  // جلب الطقس تلقائياً عند تغيير العاصمة
  useEffect(() => {
    fetchWeatherForCoords(selectedCity.lat, selectedCity.lon);
  }, [selectedCity]);

  // الكشف التلقائي لموقع المستخدم بالـ GPS
  const handleDetectGPS = () => {
    if (!navigator.geolocation) {
      alert(isAr ? 'خاصية تحديد الموقع غير مدعومة في متصفحك' : 'Geolocation is not supported by your browser');
      return;
    }
    setIsLocating(true);
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        let closest = WEATHER_CAPITALS[0];
        let minDist = Infinity;
        for (const city of WEATHER_CAPITALS) {
        ...
        fetchWeatherForCoords(latitude, longitude, isAr ? `موقعي الجغرافي (${closest.countryAr})` : `My Location (${closest.countryEn})`);
        setIsLocating(false);
      },
      () => {
        setIsLocating(false);
  ...
  // حساب الأيام والسنوات منذ انطلاق المنصة
    ...
    <div className={`w-full [perspective:1200px] ${className}`}>
      ...
      >
        ...
        >
          ...
          {/* الساعة الرقمية الكبرى تأخذ كامل عرض الشاشة */}
        ...
        >
          {/* الشريط العلوي المصغر: محدد الدولة والعاصمة (يأخذ 67% من عرض البطاقة) + زر GPS + زر العودة للساعة */}
          ...
          {/* الجزء الأوسط: درجة الحرارة + أيقونة الطقس + شبكة المؤشرات */}
          ...
          {/* الشريط السفلي المصغر: شارة الرصد + عداد العودة */}
        ...
        {/* 3. الوجه الثالث: بطاقة التواصل والمشاركة والنشرة البريدية */}
        <div 
          style={{
            backfaceVisibility: 'hidden',
            WebkitBackfaceVisibility: 'hidden',
        ...
        >
          ...
          {/* الصف الأوسط: المدير العام مسؤول النشر والناشر */}
